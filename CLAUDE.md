...
Chosen over the RP2040-Zero module: ~$3–4/panel support BOM vs ~$10 module, full pin control (clean ADC routing incl. GPIO29, 10nF caps at the pins), keeps the 4MB W25Q32JV decision (Zero is fixed 2MB), and the whole panel becomes one flat fab-assemblable PCB. Support circuit per the official "Hardware design with RP2040" minimal reference: 12MHz crystal + load caps, decoupling set, QSPI flash, **USB-C for flashing** (16-pin USB 2.0-only receptacle, **through-hole or TH-reinforced mounting required** — TH pins take the mechanical stress of repeated plug/unplug during flashing. Orientation is free (clarified 2026-07-11, supersedes the 2026-07-10 "vertical preferred"): the port is only used on the bench with the panel top off, so cable clearance is irrelevant; only constraint is receptacle body height must clear the panel platform during play (trivial, ~35mm budget). **Part as built: vertical, LCK TCF829D (LCSC C53184807)**, footprint `panel-pcb:USB_C_Receptacle_LCK_TCF829D_TEMPLATE`, assigned to J1 on both `panel-pcb` and `dual-panel`'s carrier sheet — a suitable vertical part was found on LCSC after all, superseding the 2026-07-11 pick of **GCT USB4085-GF-A** (horizontal, all-TH, stock KiCad footprint). The GCT footprint remains in both `.pretty` libraries, so reverting to horizontal is a footprint re-pick. **That swap has now been made on `hardware/dual-panel` (2026-07-28), where J1 moved to the brain board: `USB4085-GF-A`, LCSC `C7095263` (in stock, ~$0.81 — it is on LCSC, despite an early search suggesting otherwise), footprint `dual-panel:USB_C_Receptacle_GCT_USB4085_EdgeTrim`.** Production `panel-pcb` keeps the vertical LCK part. Rejected earlier: GCT USB4105 (SMT signals). 5.1kΩ pull-down on each CC pin — required for C-to-C cables; tie duplicated D+/D− pairs), **USBLC6-2SC6 ESD array on D+/D−/VBUS (U7, added 2026-07-17)** — rationale is dance-pad specific: rubber-soled shoes rubbing the panels tribocharge the pad, so USB ESD exposure isn't limited to bench plug events; sits on the connector side of the 27Ω series resistors, flow-through routed — BOOTSEL button, and a 3-pin SWD header (SWDIO/GND/SWCLK) as the firmware-independent recovery/debug path.
