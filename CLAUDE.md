...
- Underglow (resolved 2026-07-10): strips already take 12V/GND from the PSU lugs in the stock harness — the master outputs **DATA only** (5V shifter channel off the Teensy USB rail); ground reference comes via the master's PSU GND tie. **Stock connector confirmed 2026-08-08 by full pad teardown: a JST SM 3P, pinout `pin 1 = GND, pin 2 = DATA, pin 3 = 12V`** — the *reverse* of the order originally assumed. No splice needed; J2's screw terminal is final, not interim. **The pad-side half is an SMR-03V-B receptacle already fed 12V/GND from the 12V star point, with DATA arriving on a JST YLR-01V — so the master may only need to drive ONE WIRE into a mating YLP-01V rather than build an SM 3P pigtail.** Note "PSU lugs" do not exist: the stock PSU is a brick with a single YL 2-way output, and the 12V star point is physically the DC-DC converter's input screw terminals. Full stock harness record (17 WireViz drawings, wall socket to FSRs) lives in `hardware/harness/`.
