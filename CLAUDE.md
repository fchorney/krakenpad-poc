...
- **Underglow is WS2811 and behaves the OPPOSITE way to the panels' WS2815 — verified against both datasheets 2026-08-16.** WS2811 has three *independent* constant-current sinks (18.5mA each, 3 series dies per channel at 12V), so **underglow red draws ⅓ of white and colour choice scales current linearly**. WS2815 stacks its dies in series, so **panel red draws exactly the same as white and only PWM duty matters**. Current models: underglow `18.5mA × (r+g+b)/255` per group; panel **`12.9mA × max(r,g,b)/255` per pixel** — ⚠ **corrected 2026-09-08 on a REAL ASSEMBLED PANEL** (was `8.7mA`, a 48% under-estimate). The old figure was measured honestly on a `HD-12v-WS2815-144L-B-IP30` **strip cut**; the board carries JLC-placed `C5446699`, a different manufacturer and bin, and **the strip figure does not transfer**. Two-point fit from a panel drawing 0.083A at 12% duty and 0.296A at 78%, which also returns quiescent = 1.76mA/pixel against the independently documented 1.84 — so the model's *structure* is confirmed and only this constant moved. ⚠ **Do not compare 12.9 against the datasheet's 15 mA directly** — 12.9 is the PWM-scaled part with quiescent fitted separately, so full white is **12.9 + 1.76 = 14.66 mA/pixel, within 2% of the datasheet**. The 15 mA figure the whole 12V chain was sized from is therefore **CONFIRMED ON HARDWARE to ~1%** (0.43A/panel measured vs 0.44A budgeted; 6.28A pad vs 6.34A), not merely conservative. **A single global power/animation policy is therefore wrong** — model the two separately. Full treatment in `docs/UNDERGLOW.md` → "Current draw". Unlike the WS2815 figure, the long-standing underglow estimate needed **no correction**
...
- ✅ **Quiescent independently CONFIRMED 2026-09-08** at 1.76mA/pixel, fitted from a real panel — see the corrected full-scale figure above. **WS2815 quiescent is high: ~1.84mA/LED** (vs ~0.07mA for WS2812B) — ~0.41A / 5W across all 225 panel LEDs with everything dark. The price of the constant-current and dual-signal circuitry. Trivial against a 10A supply, but "all off" is not free
...
| INT (master) | **9× JST XH 2-pin vertical, B2B-XH-A (LCSC C158012)** — J3–J11; mating XHP-2 housing (C144401) + SXH-001T-P0.6N contacts (C385122). Symbol is generic `Connector_Generic:Conn_01x02` — KiCad ships no JST-specific symbol | 2 conductors each | **22 AWG** shielded twisted pair | **Replaced the 9-pos pluggable Euroblock 2026-07-26.** One connector per panel, pin 1 = INT, pin 2 = dedicated GND. Nine discrete 5.08mm screw blocks were rejected (~92mm of edge on a 77.5mm board); XH is top-entry so it needs no board edge at all. Keyed, so signal/GND cannot be swapped — that swap on a screw terminal reads as a permanently stuck press. XH-on-master vs PH-on-panel-FSR also prevents cross-plugging. Trade-off accepted: nine plugs instead of one detachable block. Silkscreened by panel position (`UL`/`U`/`UR`/`L`/`C`/`R`/`DL`/`D`/`DR`, left to right = panel 0→8). **Per-panel identification is a PRINTED LABEL under clear heat-shrink at both cable ends (decided 2026-08-31)**, not conductor color — the chosen RVSP cable comes in one color. The label reads *physical position* (`UL`/`U`/`UR`/…) — the same string as this silkscreen, so it needs no lookup table. **The six-colour two-band code (rows red/yellow/green × columns blue/white/violet, decided 2026-08-17) is now the documented FALLBACK**, kept in full because its design rules govern any colour marking here; it in turn had replaced the stock SMX 0=Red … 8=Black map, which died because nine distinguishable colours of heat-shrink are not actually sold. Labels won on the reel's arrival: the heat-shrink bin was one colour short (no violet; black and clear are both excluded as markers) while clear shrink was already on hand, and it is 36 labels against 72 bands. ⚠ **Slide the shrink on before crimping** — after termination the only retrofit markers are tape or clip-ons, and tape is rejected. Full scheme: `hardware/harness/WIRE_COLORS.md`. Slot↔panel-ID agreement is **not** assumed — the master learns the real mapping with the `'I'` identify pulse and reports mismatches; spec in docs/RS485_PROTOCOL.md |
