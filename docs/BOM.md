...
| Wire ferrules | 🛒 **SIZES CLOSED 2026-09-01 — BUY 100× 0.34 mm²** (bare; match the barrel length to the bin. 0.5 mm² needs no purchase) | domestic | **Both earlier guesses were one size too big.** **0.34 mm² (1.25 mm OD / 0.8 mm ID) ×36** for the J214 INT conductors — the reel measured 24 AWG, not the labelled 22. **0.5 mm² (1.45 mm OD / 1.05 mm ID) ×4** for master J2: the 20 AWG GND tie (DIN CSA column right, vendor labelling wrong — 20 AWG = 0.518 mm², ~80% fill) and the 22 AWG underglow DATA lead (⚠ **not** 0.34 as `PARTS.md`'s 2026-08-16 block says — the 24 AWG finding applies to the RVSP reel only; test-fit one). ~30 of each on hand, so only the 0.34 mm² falls short. ⚠ **First crimps pulled out and the ferrule was not the cause** — the HSC8 6-4 is self-adjusting with no die stations and 0.34 mm² sits at the bottom of its 0.25–6.0 mm² span; tightening its adjustment pin made the conductor snap before the ferrule releases. Both sizes verified good at that one setting. **Wago lever blocks want no ferrule at all.** Full reasoning: `hardware/harness/PARTS.md` → "Ferrules" |
