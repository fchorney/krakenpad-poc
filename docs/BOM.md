...
| DPDT slide switch (termination select) | 1 | DigiKey | **DECIDED 2026-07-11: E-Switch EG2201A** — DPDT vertical THT slide, ON-ON, 200mA/30VDC (signal-level, fine for 120Ω termination + GPIO sense). Pole 1 switches the 120Ω across A/B; pole 2 reports state to a GPIO (pull-up, LOW = terminated). Footprint pending: SnapEDA KiCad download or draw from datasheet drawing. |
